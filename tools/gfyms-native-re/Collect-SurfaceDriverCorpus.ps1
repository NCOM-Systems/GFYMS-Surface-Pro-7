...
$files = @(Get-ChildItem -LiteralPath $DriverRoot -Recurse -File |
    Where-Object { $_.Extension -in @('.sys', '.dll', '.exe', '.inf', '.cat') } |
    Sort-Object FullName)

$binaries = foreach ($file in $files | Where-Object { $_.Extension -in @('.sys', '.dll', '.exe') }) {
    [ordered]@{
        path = [IO.Path]::GetRelativePath($DriverRoot, $file.FullName) -replace '\\','/'
        extension = $file.Extension.ToLowerInvariant()
        size = $file.Length
        sha256 = Get-Sha256 $file.FullName
        signer = Get-SignerStatus $file.FullName
    }
}

$infs = foreach ($file in $files | Where-Object Extension -eq '.inf') {
    [ordered]@{
        path = [IO.Path]::GetRelativePath($DriverRoot, $file.FullName) -replace '\\','/'
        sha256 = Get-Sha256 $file.FullName
        directives = Get-TextDirectives $file.FullName
    }
}

$catalogs = foreach ($file in $files | Where-Object Extension -eq '.cat') {
    [ordered]@{
        path = [IO.Path]::GetRelativePath($DriverRoot, $file.FullName) -replace '\\','/'
        sha256 = Get-Sha256 $file.FullName
        signer = Get-SignerStatus $file.FullName
...
try {
    $pnp = @(Get-CimInstance Win32_PnPSignedDriver |
        Select-Object DeviceName, DeviceID, DriverVersion, DriverProviderName, InfName, Manufacturer |
        Sort-Object DeviceID)
} catch {
    $pnp = @([ordered]@{ error = $_.Exception.Message })
}
...
$manifest = [ordered]@{
    schema = 'gfyms.native-driver-re.v1'
